...
const EchoText = ({
  ...
  useEffect(() => {
    ...
    const handlePointerMove = event => {
      ...
      if (!frameRef.current) frameRef.current = requestAnimationFrame(renderFrame);
    };
    ...
    if (canHover) {
      // Optional local scope keeps a word cloud from following the cursor outside each word.
      const pointerTarget = pointerScope === 'self' ? root : window;
      const leaveTarget = pointerScope === 'self' ? root : document;
      pointerTarget.addEventListener('pointermove', handlePointerMove, { passive: true });
      if (pointerScope === 'self') root.addEventListener('pointerenter', handlePointerMove, { passive: true });
    ...
    const renderFrame = now => {
      ...
      const stillMoving =
        ...
        positions.some(position => Math.abs(position.x - state.targetX) > 0.01 || Math.abs(position.y - state.targetY) > 0.01) ||
        entranceProgress < 1;
  ...
  }, [blur, cursorRadius, direction, duration, ease, echoCount, fade, lag, mode, pointerScope, offset, prefersReducedMotion]);
